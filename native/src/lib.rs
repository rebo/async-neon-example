...
impl Task for DispatcherTask {
    ...
    fn perform(&self) -> Result<Self::Output, Self::Error> {
        ...
        match self.signal_receiver.recv() {
            Ok(_) => Ok(DispatchResult::Success),
            Err(_) => Err(DispatchResult::Failure),
        }
    }
    ...
    fn complete<'a, T: Scope<'a>>(
        ...
        match result {
            ...
            Ok(_) => {
                ...
                let callback: Handle<JsFunction> = self.callback_handle
                    .clone()
                    .into_handle(scope)
                    .check()
                    .unwrap();

                let msg_buffer: Handle<JsObject> = self.message_buffer_handle
                    .clone()
                    .into_handle(scope)
                    .check()
                    .unwrap();
                ...
                let get_new_messages = msg_buffer
                    .get(scope, "get_new_messages")?
                    .check::<JsFunction>()?;
                let args: Vec<Handle<JsValue>> = vec![];
                let new_messages: String = get_new_messages
                    .call(scope, msg_buffer, args)?
                    .check::<JsString>()?
                    .value();
...
pub fn perform_async_task(call: Call) -> JsResult<JsUndefined> {
    ...
    let (dispatch_signal_sender, dispatch_signal_receiver): (Sender<bool>, Receiver<bool>) =
        mpsc::channel();
